        ...
        <div className="configuracionUserMenu" ref={menuRef}>
          ...
          {menuVisible && (
            <div className="configuracionMenuDropdown">
              <button className="configuracionMenuItem" onClick={() => { setMenuVisible(false); navigate("/perfil-cajero") }}>
                Perfil
              </button>
              <button className="configuracionMenuItem" onClick={() => { setMenuVisible(false); navigate("/configuracion-cajero") }}>
                Configuración
              </button>
              <button className="configuracionMenuItem" onClick={() => { setMenuVisible(false); handleLogout() }}>
                Cerrar Sesión
              </button>
            </div>
          )}
        ...
        <div className="configuracionContentContainer">
          ...
          <form className="configuracionForm">
            <div className="configuracionFormGroup">
              <label htmlFor="nombre" className="configuracionLabel">Nombre</label>
              <input type="text" id="nombre" className="configuracionInput" placeholder="Ingresa tu nombre" />
            </div>
            <div className="configuracionFormGroup">
              <label htmlFor="email" className="configuracionLabel">Correo Electrónico</label>
              <input type="email" id="email" className="configuracionInput" placeholder="correo@ejemplo.com" />
            </div>
            <div className="configuracionFormGroup">
              <label htmlFor="password" className="configuracionLabel">Contraseña</label>
              <input type="password" id="password" className="configuracionInput" placeholder="********" />
            </div>
            <div className="configuracionButtonContainer">
              <button type="submit" className="configuracionSaveButton">Guardar Cambios</button>
              <button type="button" className="gerenteConfiguracionReturnButton" onClick={() => navigate(-1)}>Regresar</button>
            </div>
          </form>
        </div>
      </main>
