...
$usuarioData = @{
    ...
    password = "password123"  # CAMPO CAMBIADO: password en lugar de passwordHash
...
try {
    ...
    Write-Host "`n🎉 EXITO: Usuario creado exitosamente!" -ForegroundColor Green -BackgroundColor Black
    Write-Host "Respuesta del servidor:" -ForegroundColor Green
    Write-Host ($response | ConvertTo-Json -Depth 2) -ForegroundColor Green
      Write-Host "`n✅ SOLUCION ENCONTRADA:" -ForegroundColor Green -BackgroundColor Black
    ...
    Write-Host "`n❌ FALLO PERSISTENTE" -ForegroundColor Red
    ...
    if ($errorBody -like "*password*") {
        Write-Host "`n🔍 DIAGNOSTICO ADICIONAL NECESARIO:" -ForegroundColor Yellow
